...
def compileAll(row,i):
	
	with open('../scripts/records/records_'+str(i)+'.csv','a') as csvfile:
...
def textSorter(i):
	try:
		...
		for j in data:	
			...
			row.append(setA)
			try:
    				os.remove('../scripts/records/records_'+str(i)+'.csv')
			except OSError:
    				pass
		compileAll(row,i)
...
def main():
	#Removing existing list of antenna ids
	try:
